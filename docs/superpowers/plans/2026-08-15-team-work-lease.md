...
- [x] **Step 1: Build isolated roster and pack helpers**
...
- [x] **Step 2: Add a first failing claim/ack chain test**
...
- [x] **Step 3: Run only the first test and verify red**
...
- [x] **Step 4: Add negative and concurrency cases before implementation**
...
- [x] **Step 5: Add state, PR-link, writeback, and isolation cases**
...
- [x] **Step 1: Add the new tables and indexes**
...
- [x] **Step 2: Add append-only triggers**
...
- [x] **Step 3: Verify the schema directly before command parsing exists**
...
- [x] **Step 1: Extend the Bash wrapper without changing read-only paths**
...
- [x] **Step 2: Add actor and mutation argument validators in Node**
...
- [x] **Step 3: Implement a quoted SQLite transaction runner**
...
- [x] **Step 4: Implement claim and active-holder mutations**

`claim` inserts at the contract item's revision or takes only an absent/expired
lease while incrementing revision. `ack`, `renew`, and `release` require an
exact unexpired lease owner. `set-state`, `link-pr`, and `writeback` accept
either that holder or an exact roster manager, while all mutations require
matching stored contract/envelope digests. `ack` sets `acknowledged`; `renew`
refreshes expiry; `release` clears lease; `set-state` sets an allowed state;
`link-pr` appends one distinct local relation; and `writeback` appends local
evidence.

- [x] **Step 5: Run the state test file until green**
...
- [x] **Step 6: Commit the mutation engine**
...
- [x] **Step 1: Add README usage and safety rules**

Place mutation examples immediately after the existing read-only team-work
section. Explain exact owner/manager authorization: ACK/renew/release remain
active-holder-only, while manager-authorized state/link/writeback mutations do
not take another seat's lease. Also explain reclaim after expiry, revision
history, and the fact that `link-pr`/`writeback` record local state rather than
writing to GitHub.
...
- [x] **Step 3: Record actual verification evidence and commit docs**
...
- `BATS_SHELL=/bin/bash bats --print-output-on-failure tests/test_team_work_state.bats --filter 'owner claim and ack'`
  initially failed with the expected `team-work.sh` usage diagnostic before the
  mutation command existed.
- `BATS_SHELL=/bin/bash bats --print-output-on-failure tests/test_team_work_state.bats`
  passed all 8 state, authority, expiry, revision, and non-interference tests.
- An isolated `mktemp -d` store initialized by `scripts/internal/init-db.sh`
  contained both `team_work_current` and `team_work_revisions`.
- `node --check scripts/lib/team-work.js` and
  `shellcheck -s bash -e SC1091 scripts/team-work.sh scripts/internal/init-db.sh scripts/lib/storage.sh`
  passed.
- CI-equivalent four-way sharding over all 47 Bats files (135 tests) completed
  with `shard 1 exit 0`, `shard 2 exit 0`, `shard 3 exit 0`, and
  `shard 4 exit 0`.
