...
then enforced. When none is set, exactly one valid agmsg registration is
required: `claude-code` selects `kappaseijin4claude` and `codex` selects
`kappaseijin4codex`. Non-exact identity, malformed or unreadable roster
output, an unsupported runtime, or token lookup failure stops with a nonzero
status before the static policy or real `gh` is run. Tokens are never printed
or logged.
...
By default `despawn <name>` is **graceful**: it sends a `ctrl:despawn` control message to `<name>`, whose watcher drops its own role (releasing the actas lock and registration) and closes its own tmux pane — ending the agent. It blocks until the role is released, up to `--timeout <secs>` (default 30), then prints `status=ok`. If the member's watcher never responds it prints `status=timeout` and exits 3 — retry with `--force`. If lock state cannot be confirmed, it prints `status=unavailable operation=lock-state`, exits nonzero, and does not claim success or remove the placement record.

`--force` skips the message and tears the member down from the placement recorded at spawn time: it kills the member's tmux pane/window and drops its registration. Use it when the member's watcher can't respond — a dead watcher, or a **codex** member (no Monitor, so graceful has nothing to act on). It prints `status=forced` only when reset and lock release succeed; if either fails, it prints `status=partial operation=registration-reset` or `status=partial operation=lock-release`, exits nonzero, and keeps the placement record for a retry. A member started by hand (no spawn placement record) can't be `--force`d; despawn says so and leaves it for you to close.
