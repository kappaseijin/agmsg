...
`actas <name>` は**セッション間で排他的**だ — 現在のランタイムでローカル登録されている全チームについて、完全一致する `<name>` の送受信へ切り替え、該当するロックを取得する。別セッションが1つでも保持している場合は拒否する。`drop` は現在のプロジェクトの登録だけを削除し、このセッションがその名前について全チームで保持しているロックを解放する。ロックが固まってしまった場合は、保持しているセッションからロールをdropするか、そのセッションを終了すること。
...
デフォルトで `despawn <name>` は**グレースフル**だ — `<name>` に `ctrl:despawn` 制御メッセージを送信し、そのウォッチャーが自分のロール（actasロックと登録の解放）を解除し、自分のtmuxペインを閉じてエージェントを終了させる。ロールが解放されるまでブロックし、`--timeout <secs>`（デフォルト30）が上限、その後 `status=ok` を表示する。メンバーのウォッチャーが応答しない場合は `status=timeout` を表示して終了コード3 — `--force` で再試行すること。ロック状態を確認できない場合は `status=unavailable operation=lock-state` を表示して非0で終了し、成功扱いや配置記録の削除を行わない。

`--force` はメッセージ送信をスキップし、spawn時に記録された配置情報からメンバーを終了させる — メンバーのtmuxペイン/ウィンドウをkillし、登録を削除する。メンバーのウォッチャーが応答できない場合（ウォッチャーが死んでいる、または**codex**メンバー — Monitorがないためグレースフルには何も反応するものがない）に使う。resetとロック解放が成功した場合だけ `status=forced` を表示する。どちらかが失敗した場合は `status=partial operation=registration-reset` または `status=partial operation=lock-release` を表示して非0で終了し、再試行できるよう配置記録を保持する。手動で起動されたメンバー（spawnの配置記録がない）は `--force` できない — despawnがその旨を伝え、あなた自身が閉じることになる。
...
/agmsg mode                             — 現在のモードを表示
/agmsg actas <name>                     — 別のロールに切り替え（ローカル登録チームで同名を購読。必要ならこのプロジェクトに作成）
...
スクリプトツリーは staging してから rename で配置するため、実行中の
watcher が開いているファイルを途中で上書きしない。更新後は、実行中の
agent セッションを再起動して新しいスクリプトを読み込ませる。安全に
移動できない場合はエラーとして停止し、in-place 上書きへフォールバック
しない。実行中プロセスを中断しない性質は macOS/Linux で検証済みだが、
Windows/MSYS2 の挙動は未検証の既知の制約である。
