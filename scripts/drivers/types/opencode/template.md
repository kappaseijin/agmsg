...
Otherwise, run: `~/.agents/skills/__SKILL_NAME__/scripts/whoami.sh "$(pwd)" opencode`
...
→ Show the user the available teams from the output, then:
  ...
  3. **You MUST use join.sh** — run: `~/.agents/skills/__SKILL_NAME__/scripts/join.sh <team> <agent_name> opencode "$(pwd)"`
...
→ No exact registration exists for this project, but there are same-type agent names registered elsewhere.
  ...
  4. Run: `~/.agents/skills/__SKILL_NAME__/scripts/join.sh <team> <agent_name> opencode "$(pwd)"`
...
1. Run: `~/.agents/skills/__SKILL_NAME__/scripts/delivery.sh set off opencode "$(pwd)"`
