...
class FakeI1:
    """Stand-in for pilot-gate-i1 backed by real team configs and sqlite."""

    def __init__(self, test, db: Path, events: list):
        self.test = test
        self.db = db
        self.events = events
        self.provider_calls = []
        self.registered = []
        # Scenario knobs.
        self.provider_state = {"control": "queued", "fault": "queued"}
        self.storage_writes = {"control": 1, "fault": 1}
        self.message_ids = {"control": "msg-control", "fault": "msg-fault"}
        self.response_overrides = {"control": {}, "fault": {}}
        make_events_db(db, [])

    def sanitize_env(self, environ):
        self.events.append("sanitize_env")
        return {"PATH": environ.get("PATH", ""), "SANITIZED": "1"}

    def register_fixture_member(
        self, gate_repo, team, agent, project, role, env, mutation_log
    ):
        self.events.append(f"register:{role}")
        self.registered.append(
            (gate_repo, team, agent, project, role, dict(env), mutation_log)
        )
        path = gate_repo / "teams" / team / "config.json"
        if path.exists():
            config = json.loads(path.read_text())
        else:
            config = {"name": team, "agents": {}}
        config["agents"][agent] = {"type": "claude-code", "role": role}
        write_team(gate_repo, team, config)

    def storage_db(self, gate_repo, team, env):
        self.events.append("storage_db")
        return self.db

    def provider_call(self, provider, args, gate_repo, env, stdin, check):
        _, team, sender, recipient, request_id, body = args
        case = json.loads(body)["kind"].rsplit("-", 1)[1]
        self.events.append(f"provider:{case}")
        self.provider_calls.append(
            (provider, list(args), gate_repo, dict(env), stdin, check)
        )
        message_id = self.message_ids[case]
        rows = [
            (message_id, f"legacy-{case}-{n}", "message_sent", team,
             sender, recipient, body, "2026-09-11T00:00:00.000Z")
            for n in range(self.storage_writes[case])
        ]
        if rows:
            self.add_rows(rows)
        response = {
            "state": self.provider_state[case],
            "team": team,
            "from": sender,
            "to": recipient,
            "messageId": message_id,
        }
        response.update(self.response_overrides[case])
        return response
...
class FakeWatcher:
    """Scripted GateWatcher replacement; one instance per start phase."""

    instances: list = []
    plan: dict = {}
    events: list = []

    def __init__(self, *, watch_script, project, recipient, session_id,
                 env, artifact):
        ...
        self.index = len(FakeWatcher.instances)
        self.phase = ("control", "recovery")[self.index]
        self.running = False
        self.started = False
        self.stop_calls = 0
        self.lines = []
    ...
    def start(self):
        FakeWatcher.events.append(f"watcher-start:{self.phase}")
        hook = FakeWatcher.plan.get(f"{self.phase}_start")
        if hook is not None:
            hook(self)
        self.started = True
        self.running = True
    ...
    def wait_for_token(self, token, timeout):
        FakeWatcher.events.append(f"watcher-wait:{self.phase}")
        self.waited = (token, timeout)
        count = FakeWatcher.plan.get(f"{self.phase}_count", 1)
        self.lines.extend(f"notify {token}" for _ in range(count))
        return FakeWatcher.plan.get(f"{self.phase}_observed", count >= 1)
    ...
    def stop(self):
        FakeWatcher.events.append(f"watcher-stop:{self.phase}")
        self.stop_calls += 1
        if FakeWatcher.plan.get(f"{self.phase}_stop_works", True):
            self.running = False
        leak = FakeWatcher.plan.get(f"{self.phase}_leak_after_stop")
        if leak:
            self.lines.append(f"late notify {leak}")


class PilotGateF5RoundC(unittest.TestCase):
    """run_f5 sequencing: containment, control, fault, recovery."""

    RUN_ID = "round-c-run"
    GATE = "agmsg-g4gate-f5-c"
    LIVE = "live-team"

    def setUp(self):
        self._tmp = tempfile.TemporaryDirectory()
        self.root = Path(self._tmp.name).resolve()
        self.run_root = self.root / "run-root"
        self.gate_repo = self.run_root / "repo"
        self.claude_config = self.run_root / "claude"
        self.artifact_dir = self.root / "artifacts"
        self.artifact = self.artifact_dir / "F5"
        scripts = self.gate_repo / "scripts"
        scripts.mkdir(parents=True)
        self.claude_config.mkdir()
        for name in ("p2-provider.sh", "watch.sh", "join.sh"):
            path = scripts / name
            path.write_text("#!/bin/sh\nexit 99\n", encoding="utf-8")
            path.chmod(0o700)
        write_team(self.gate_repo, "live", team_config(self.LIVE, "someone"))

        self.events = []
        self.i1 = FakeI1(self, self.root / "events.db", self.events)
        FakeWatcher.instances = []
        FakeWatcher.plan = {}
        FakeWatcher.events = self.events

        self.sender = F5.safe_token("agmsg_gate_f5_sender", self.RUN_ID)
        self.recipient = F5.safe_token(
            "agmsg_gate_f5_recipient", self.RUN_ID
        )
        self.control_token = F5.safe_token("AGMSG_F5_CONTROL", self.RUN_ID)
        self.fault_token = F5.safe_token("AGMSG_F5_FAULT", self.RUN_ID)
        self.session = F5.safe_token("agmsg-g4-f5-watch", self.RUN_ID)

    def tearDown(self):
        self._tmp.cleanup()

    def reset(self):
        """Fresh fixture for a subTest, without leaking the old tempdir."""
        self._tmp.cleanup()
        self.setUp()
    ...
    def args(self, **overrides):
        values = {
            ...
            "gate_team": self.GATE,
            "claude_config": str(self.claude_config),
        }
        values.update(overrides)
        return argparse.Namespace(**values)

    def run_f5(self, **overrides):
        sleep = mock.Mock(
            side_effect=lambda seconds: self.events.append(
                f"sleep:{seconds}"
            )
        )
        with mock.patch.object(
            F5, "load_module", return_value=self.i1
        ) as load, mock.patch.object(
            F5, "GateWatcher", FakeWatcher
        ), mock.patch.object(F5.time, "sleep", sleep):
            rc = F5.run_f5(self.args(**overrides))
        self.load = load
        self.sleep = sleep
        return rc

    def read(self, *parts):
        return json.loads(self.artifact.joinpath(*parts).read_text())

    def verdicts(self, checks):
        return {item["name"]: item["verdict"] for item in checks}

    # --- happy path ------------------------------------------------------

    def test_pass_path_returns_0_and_records_all_phases(self):
        rc = self.run_f5()
        ...
        self.assertEqual(
            ...
            [
                ...
                "register:recipient",
                "storage_db",
                "watcher-start:control",
                "provider:control",
                "watcher-wait:control",
                # The fault message is sent only after the consumer
                # is stopped, and observed over the full window.
                "watcher-stop:control",
                "provider:fault",
                f"sleep:{F5.FAULT_OBSERVATION_SECONDS}",
                "watcher-start:recovery",
                "watcher-wait:recovery",
                "watcher-stop:control",
                "watcher-stop:recovery",
            ],
        )
        self.sleep.assert_called_once_with(F5.FAULT_OBSERVATION_SECONDS)

        final = self.read("result.json")
        self.assertEqual(final["verdict"], "pass")
        self.assertEqual(final["check"], "F5")
        self.assertEqual(final["runId"], self.RUN_ID)
        self.assertEqual(
            final["faultMethod"], "gate-owned-recipient-watcher-stopped"
        )
        self.assertEqual(final["gateTeam"], self.GATE)
        self.assertEqual(final["sender"], self.sender)
        self.assertEqual(final["recipient"], self.recipient)
        self.assertEqual(
            self.verdicts(final["checks"]),
            {
                ...
                "recovery-delivered-exactly-once": "pass",
                "control-and-fault-message-ids-distinct": "pass",
            },
        )
        self.assertEqual(final["control"], self.read("control", "result.json"))
        self.assertEqual(final["fault"], self.read("fault", "result.json"))
        self.assertEqual(
            final["recovery"], self.read("recovery", "result.json")
        )

        control = final["control"]
        self.assertEqual(control["token"], self.control_token)
        self.assertEqual(control["messageId"], "msg-control")
        self.assertEqual(control["deliveryCount"], 1)
        self.assertEqual(control["storage"]["count"], 1)
        self.assertEqual(
            set(self.verdicts(control["checks"]).values()), {"pass"}
        )

        fault = final["fault"]
        self.assertEqual(fault["deliveryState"], "stale/consumer_stopped")
        self.assertEqual(fault["deliveryCountBeforeRecovery"], 0)
        self.assertEqual(
            self.verdicts(fault["checks"]),
            {
                "fault-watcher-stopped": "pass",
                "fault-provider-still-accepts-message": "pass",
                "fault-message-persisted-exactly-once": "pass",
                "fault-not-delivered-while-consumer-stopped": "pass",
                "fault-stale-indication-observable": "pass",
            },
        )

        recovery = final["recovery"]
        self.assertEqual(recovery["watchSessionId"], self.session)
        self.assertEqual(recovery["messageId"], "msg-fault")
        self.assertEqual(recovery["deliveryCount"], 1)
        self.assertEqual(
            self.verdicts(recovery["checks"]),
            {
                "recovery-same-watch-session-id": "pass",
                "recovery-pending-message-observed": "pass",
                "recovery-same-message-identity": "pass",
                "recovery-delivered-exactly-once": "pass",
                "recovery-duplicate-storage-write-zero": "pass",
                "all-provider-destinations-inside-disposable-team": "pass",
            },
        )

    def test_pass_path_wires_watchers_providers_and_registration(self):
        self.assertEqual(self.run_f5(), 0)

        runtime = self.gate_repo / ".agmsg-gate" / "f5"
        mutation_log = self.artifact / "mutation-log.jsonl"
        self.assertEqual(
            [(r[1], r[2], r[3], r[4], r[6]) for r in self.i1.registered],
            [
                (self.GATE, self.sender, runtime / "sender", "sender",
                 mutation_log),
                (self.GATE, self.recipient, runtime / "recipient",
                 "recipient", mutation_log),
            ],
        )
        env = self.i1.registered[0][5]
        self.assertEqual(env["CLAUDE_CONFIG_DIR"], str(self.claude_config))
        self.assertEqual(env["SANITIZED"], "1")
        self.assertEqual(
            self.load.call_args.args,
            (F5_HELPER.parent / "pilot-gate-i1.py", "pilot_gate_i1"),
        )

        control, recovery = FakeWatcher.instances
        for watcher in (control, recovery):
            self.assertEqual(watcher.watch_script,
                             self.gate_repo / "scripts" / "watch.sh")
            self.assertEqual(watcher.project, runtime / "recipient")
            self.assertEqual(watcher.recipient, self.recipient)
            # Recovery reuses the same delivery identity.
            self.assertEqual(watcher.session_id, self.session)
            self.assertEqual(watcher.env["CLAUDE_CONFIG_DIR"],
                             str(self.claude_config))
            self.assertEqual(watcher.settled,
                             F5.POST_DELIVERY_SETTLE_SECONDS)
            self.assertFalse(watcher.running)
        self.assertEqual(control.artifact,
                         self.artifact / "control" / "watcher")
        self.assertEqual(recovery.artifact,
                         self.artifact / "recovery" / "watcher")
        self.assertEqual(control.waited,
                         (self.control_token, F5.DELIVERY_TIMEOUT_SECONDS))
        self.assertEqual(recovery.waited,
                         (self.fault_token, F5.DELIVERY_TIMEOUT_SECONDS))

        provider = self.gate_repo / "scripts" / "p2-provider.sh"
        self.assertEqual(len(self.i1.provider_calls), 2)
        for (call, case, token, request) in zip(
            self.i1.provider_calls,
            ("control", "fault"),
            (self.control_token, self.fault_token),
            ("f5-control-request", "f5-fault-request"),
        ):
            path, argv, repo, call_env, stdin, check = call
            self.assertEqual(path, provider)
            self.assertEqual(repo, self.gate_repo)
            self.assertEqual((stdin, check), (None, False))
            self.assertEqual(
                argv[:5],
                ["message-send", self.GATE, self.sender, self.recipient,
                 F5.safe_token(request, self.RUN_ID)],
            )
            self.assertEqual(
                json.loads(argv[5]),
                {"schemaVersion": 1, "kind": f"f5-notification-{case}",
                 "runId": self.RUN_ID, "token": token},
            )

        records = [
            json.loads(line) for line in
            (self.artifact / "provider-argv.jsonl").read_text().splitlines()
        ]
        self.assertEqual(len(records), 2)
        self.assertTrue(all(r["safe"] is True for r in records))

        containment = self.read("containment.json")
        self.assertEqual(containment["verdict"], "pass")
        self.assertEqual(containment["check"], "F5-containment")
        self.assertEqual(containment["liveTeamNames"], [self.LIVE])

    def test_stale_logs_from_previous_run_are_removed(self):
        self.artifact.mkdir(parents=True)
        (self.artifact / "mutation-log.jsonl").write_text("stale\n")
        (self.artifact / "provider-argv.jsonl").write_text(
            '{"safe": false, "liveTeamArgvHits": ["x"]}\n'
        )
        self.assertEqual(self.run_f5(), 0)
        records = (self.artifact / "provider-argv.jsonl").read_text()
        self.assertNotIn('"safe":false', records.replace(" ", ""))
        self.assertEqual(len(records.splitlines()), 2)

    # --- preconditions ---------------------------------------------------

    def test_recipient_already_in_live_team_fails_before_any_mutation(self):
        write_team(
            self.gate_repo, "live2", team_config("live-2", self.recipient)
        )
        self.assertEqual(self.run_f5(), 1)
        self.assertEqual(
            self.read("containment.json"),
            {
                "schemaVersion": 1,
                "verdict": "fail",
                "reason": "recipient_already_exists_in_live_team",
                "recipient": self.recipient,
                "collisions": ["live-2"],
            },
        )
        self.assertEqual(self.i1.registered, [])
        self.assertEqual(self.i1.provider_calls, [])
        self.assertEqual(FakeWatcher.instances, [])

    def test_existing_gate_team_with_recipient_is_not_a_live_collision(self):
        write_team(
            self.gate_repo, self.GATE, team_config(self.GATE, self.recipient)
        )
        self.assertEqual(self.run_f5(), 0)

    def test_unreadable_live_team_agent_map_aborts_before_registration(self):
        write_team(self.gate_repo, "bad", {"name": "bad", "agents": [1]})
        with self.assertRaisesRegex(
            RuntimeError, "cannot identify live-team agent map: bad"
        ):
            self.run_f5()
        self.assertEqual(self.i1.registered, [])

    def test_gate_repo_outside_run_root_is_rejected(self):
        other_root = self.root / "elsewhere"
        other_root.mkdir()
        with self.assertRaises(ValueError):
            self.run_f5(run_root=str(other_root))
        self.assertEqual(self.i1.registered, [])

    def test_missing_or_non_executable_script_is_rejected(self):
        for name in ("p2-provider.sh", "watch.sh", "join.sh"):
            with self.subTest(script=name):
                path = self.gate_repo / "scripts" / name
                path.chmod(0o600)
                try:
                    with self.assertRaisesRegex(
                        RuntimeError, "not regular executable"
                    ):
                        self.run_f5()
                finally:
                    path.chmod(0o700)
    ...
    def test_missing_claude_config_is_rejected(self):
        with self.assertRaises(FileNotFoundError):
            self.run_f5(claude_config=str(self.root / "nope"))

    def test_containment_verdict_maps_to_exit_code_without_sending(self):
        for verdict, rc in (("fail", 1), ("unknown", 2)):
            with self.subTest(verdict=verdict):
                FakeWatcher.instances = []
                self.i1.provider_calls = []
                with mock.patch.object(
                    F5, "prove_containment",
                    return_value={"verdict": verdict, "checks": []},
                ):
                    self.assertEqual(self.run_f5(), rc)
                self.assertEqual(self.read("containment.json")["verdict"],
                                 verdict)
                self.assertEqual(self.i1.provider_calls, [])
                self.assertEqual(FakeWatcher.instances, [])
    ...
    def assert_stopped_at_control(self, rc, expected_rc, failing):
        self.assertEqual(rc, expected_rc)
        final = self.read("result.json")
        self.assertEqual(final["reason"], "control_not_pass")
        self.assertEqual(final["verdict"],
                         "fail" if expected_rc == 1 else "unknown")
        self.assertNotIn("fault", final)
        verdicts = self.verdicts(final["control"]["checks"])
        for name in failing:
            self.assertNotEqual(verdicts[name], "pass", name)
        self.assertEqual(
            [c[1][4] for c in self.i1.provider_calls],
            [F5.safe_token("f5-control-request", self.RUN_ID)],
        )
        self.assertEqual(len(FakeWatcher.instances), 1)
        self.assertFalse(FakeWatcher.instances[0].running)
        self.sleep.assert_not_called()

    def test_control_not_delivered_fails(self):
        FakeWatcher.plan = {"control_count": 0}
        self.assert_stopped_at_control(
            self.run_f5(), 1,
            ["control-delivery-observed", "control-delivered-exactly-once"],
        )

    def test_control_delivered_twice_fails(self):
        FakeWatcher.plan = {"control_count": 2}
        self.assert_stopped_at_control(
            self.run_f5(), 1, ["control-delivered-exactly-once"]
        )

    def test_control_observation_unknown_maps_to_2(self):
        FakeWatcher.plan = {"control_observed": None}
        self.assert_stopped_at_control(
            self.run_f5(), 2, ["control-delivery-observed"]
        )

    def test_control_not_queued_fails(self):
        self.i1.provider_state["control"] = "rejected"
        self.assert_stopped_at_control(
            self.run_f5(), 1, ["control-provider-queued"]
        )

    def test_control_storage_missing_or_duplicated_fails(self):
        for writes in (0, 2):
            with self.subTest(writes=writes):
                self.reset()
                self.i1.storage_writes["control"] = writes
                self.assert_stopped_at_control(
                    self.run_f5(), 1,
                    ["control-message-identity-matches-storage"],
                )

    def test_control_without_message_id_fails_without_storage_query(self):
        self.i1.message_ids["control"] = None
        with mock.patch.object(
            F5, "storage_message_observation",
            wraps=F5.storage_message_observation,
        ) as observe:
            rc = self.run_f5()
        self.assert_stopped_at_control(
            rc, 1,
        ...
        self.assertEqual(self.read("control", "result.json")["storage"],
                         {"count": 0, "rows": []})

    # Each field of the provider's "queued" answer is checked on its
    # own: only one field is wrong per case, and storage still holds
    # exactly one matching row, so no other check can mask it.
    BAD_FIELDS = (
        ("state", "sent"),
        ("team", "live-team"),
        ("from", "someone-else"),
    ...
    def test_control_queued_checks_every_response_field(self):
        for field, value in self.BAD_FIELDS:
            with self.subTest(field=field):
                self.reset()
                self.i1.response_overrides["control"] = {field: value}
                if field == "messageId":
                    # An empty id cannot be looked up; storage check fails
                    # too, which is expected for this field.
                    failing = ["control-provider-queued",
                               "control-message-identity-matches-storage"]
                else:
                    failing = ["control-provider-queued"]
                rc = self.run_f5()
                self.assert_stopped_at_control(rc, 1, failing)
                verdicts = self.verdicts(
                    self.read("result.json")["control"]["checks"]
                )
                self.assertEqual(
                    verdicts["control-delivery-observed"], "pass"
                )

    def test_fault_queued_checks_every_response_field(self):
        for field, value in self.BAD_FIELDS:
            with self.subTest(field=field):
                self.reset()
                self.i1.response_overrides["fault"] = {field: value}
                failing = ["fault-provider-still-accepts-message"]
                if field == "messageId":
                    failing.append("fault-message-persisted-exactly-once")
                self.assert_stopped_at_fault(self.run_f5(), failing)
                verdicts = self.verdicts(
                    self.read("fault", "result.json")["checks"]
                )
                self.assertEqual(verdicts["fault-watcher-stopped"], "pass")
                self.assertEqual(
                    verdicts["fault-not-delivered-while-consumer-stopped"],
                    "pass",
                )

    # --- fault phase -----------------------------------------------------

    def assert_stopped_at_fault(self, rc, failing):
        self.assertEqual(rc, 1)
        final = self.read("result.json")
        self.assertEqual(final["reason"], "fault_not_pass")
        self.assertEqual(final["verdict"], "fail")
        self.assertNotIn("recovery", final)
        fault = final["fault"]
        self.assertEqual(fault["deliveryState"], "unproved")
        verdicts = self.verdicts(fault["checks"])
        for name in failing:
            self.assertEqual(verdicts[name], "fail", name)
        self.assertEqual(verdicts["fault-stale-indication-observable"],
                         "fail")
        self.assertEqual(len(FakeWatcher.instances), 1)
        self.assertFalse(
            self.artifact.joinpath("recovery", "result.json").exists()
        )

    def test_fault_fails_when_consumer_did_not_stop(self):
        FakeWatcher.plan = {"control_stop_works": False}
        self.assert_stopped_at_fault(
            self.run_f5(), ["fault-watcher-stopped"]
        )

    def test_fault_fails_when_message_delivered_while_consumer_stopped(self):
        FakeWatcher.plan = {"control_leak_after_stop": self.fault_token}
        rc = self.run_f5()
        self.assert_stopped_at_fault(
            rc, ["fault-not-delivered-while-consumer-stopped"]
        )
        self.assertEqual(
            self.read("fault", "result.json")["deliveryCountBeforeRecovery"],
            1,
        )

    def test_fault_fails_when_provider_rejects(self):
        self.i1.provider_state["fault"] = "rejected"
        self.assert_stopped_at_fault(
            self.run_f5(), ["fault-provider-still-accepts-message"]
        )

    def test_fault_fails_when_storage_missing_or_duplicated(self):
        for writes in (0, 2):
            with self.subTest(writes=writes):
                self.reset()
                self.i1.storage_writes["fault"] = writes
                self.assert_stopped_at_fault(
                    self.run_f5(), ["fault-message-persisted-exactly-once"]
                )

    def test_fault_without_message_id_fails(self):
        self.i1.message_ids["fault"] = None
        self.assert_stopped_at_fault(
            self.run_f5(),
            ["fault-provider-still-accepts-message",
             "fault-message-persisted-exactly-once"],
        )

    # --- recovery phase --------------------------------------------------

    def recovery_checks(self):
        return self.verdicts(self.read("recovery", "result.json")["checks"])

    def test_recovery_not_observed_fails(self):
        FakeWatcher.plan = {"recovery_count": 0}
        self.assertEqual(self.run_f5(), 1)
        checks = self.recovery_checks()
        self.assertEqual(checks["recovery-pending-message-observed"], "fail")
        self.assertEqual(checks["recovery-delivered-exactly-once"], "fail")
        self.assertEqual(
            self.verdicts(self.read("result.json")["checks"])[
                "recovery-delivered-exactly-once"
            ],
            "fail",
        )

    def test_recovery_duplicate_delivery_fails(self):
        FakeWatcher.plan = {"recovery_count": 2}
        self.assertEqual(self.run_f5(), 1)
        self.assertEqual(
            self.recovery_checks()["recovery-delivered-exactly-once"], "fail"
        )
    ...
    def test_recovery_unknown_maps_to_final_unknown_and_rc_2(self):
        ...
        self.assertEqual(
            self.recovery_checks()["recovery-pending-message-observed"],
            "unknown",
        )
        final = self.read("result.json")
    ...
    def test_recovery_duplicate_storage_write_fails(self):
        def duplicate(watcher):
            case_body = json.dumps(
                {"schemaVersion": 1, "kind": "f5-notification-fault",
                 "runId": self.RUN_ID, "token": self.fault_token},
                separators=(",", ":"), sort_keys=True,
            )
            self.i1.add_rows([
                ("msg-fault", "legacy-dup", "message_sent", self.GATE,
                 self.sender, self.recipient, case_body, "t"),
            ])

        FakeWatcher.plan = {"recovery_start": duplicate}
        self.assertEqual(self.run_f5(), 1)
        checks = self.recovery_checks()
        self.assertEqual(checks["recovery-same-message-identity"], "fail")
        self.assertEqual(checks["recovery-duplicate-storage-write-zero"],
                         "fail")
        recovery = self.read("recovery", "result.json")
        detail = [c for c in recovery["checks"]
                  if c["name"] == "recovery-duplicate-storage-write-zero"]
        self.assertEqual(detail[0]["detail"],
                         {"persistentWrites": 2, "duplicateWrites": 1})

    def test_provider_log_with_live_team_hit_fails(self):
        def taint(watcher):
            F5.append_jsonl(
                self.artifact / "provider-argv.jsonl",
                {"kind": "provider", "safe": True,
                 "liveTeamArgvHits": [self.LIVE]},
            )

        FakeWatcher.plan = {"recovery_start": taint}
        self.assertEqual(self.run_f5(), 1)
        self.assertEqual(
    ...
    def test_provider_log_with_unsafe_or_nonobject_record_fails(self):
        for record in ({"kind": "provider", "safe": False}, ["not", "dict"],
                       {"kind": "provider"}):
            with self.subTest(record=record):
                self.reset()

                def taint(watcher, record=record):
                    F5.append_jsonl(
                        self.artifact / "provider-argv.jsonl", record
                    )
    ...
    def test_unreadable_provider_log_is_unknown(self):
        def corrupt(watcher):
            with open(self.artifact / "provider-argv.jsonl", "a") as fh:
                fh.write("{not json\n")
    ...
    def test_identical_control_and_fault_message_ids_fail(self):
        self.i1.message_ids["fault"] = "msg-same"
        self.i1.message_ids["control"] = "msg-same"
        # Storage rows differ by body, so each phase still sees count 1.
        self.assertEqual(self.run_f5(), 1)
        final = self.read("result.json")
        self.assertEqual(
            self.verdicts(final["checks"])[
                "control-and-fault-message-ids-distinct"
            ],
            "fail",
        )
        for phase in ("control", "fault", "recovery"):
            self.assertEqual(final[phase]["verdict"], "pass", phase)

    # --- cleanup ---------------------------------------------------------

    def test_watchers_are_stopped_when_recovery_start_raises(self):
        def boom(watcher):
            raise RuntimeError("recovery start failed")

        FakeWatcher.plan = {"recovery_start": boom}
        with self.assertRaisesRegex(RuntimeError, "recovery start failed"):
            self.run_f5()
        control, recovery = FakeWatcher.instances
        self.assertEqual(control.stop_calls, 2)
        self.assertEqual(recovery.stop_calls, 1)

    def test_cleanup_stop_errors_do_not_mask_result(self):
        original_stop = FakeWatcher.stop
        calls = []

        def flaky_stop(watcher):
            calls.append(watcher.phase)
            original_stop(watcher)
            if len(calls) > 1:
                raise RuntimeError("stop failed during cleanup")

        with mock.patch.object(FakeWatcher, "stop", flaky_stop):
            self.assertEqual(self.run_f5(), 0)
        self.assertEqual(calls, ["control", "control", "recovery"])
