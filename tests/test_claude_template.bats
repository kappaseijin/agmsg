...
@test "Claude template tells actas and drop not to treat unknown or off as silently deliberate except turn (#175)" {
  ...
  count_unrecognized=$(grep -c 'mode: unknown (unrecognized: \.\.\.)' "$TEMPLATE")
  [ "$count_unrecognized" -eq 2 ] \
    || { echo "expected 2 occurrences of the unknown wording (actas + drop), found $count_unrecognized" >&2; return 1; }
